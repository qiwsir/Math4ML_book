...
故 $\pmb{T}(\pmb{w}_1),\cdots,\pmb{T}(\pmb{w}_r)$ 是线性无关的向量集合，是 $\rank(\pmb{T})$ 的基。
...
因为矩阵的行秩等于列秩，即 $\rank\pmb{A}=\dim C(\pmb{A})=\dim C(\pmb{A}^{\text{T}})$ ，于是“秩—零化度定理”可以写成：
...
根据前面的假设 $r=\rank\pmb{A}$ ，可以假设 $\sigma_1\ge\sigma_2\ge\cdots\sigma_r\gt0$ ，且 $\sigma+{r+1}=\cdots=\sigma_n=0$ ，则 $\rank(\pmb{A}^{\text{T}}\pmb{A})=r$ 。
...
- $r=\rank\pmb{A}=\rank\pmb{A}^{\text{T}}=\rank(\pmb{A}^{\text{T}}\pmb{A})=\rank(\pmb{AA}^{\text{T}})$
...
| 满射：$R(\pmb{T})=\mathbb{W}$ ，即 $\rank\pmb{T}=\dim\mathbb{W}$ | 满行秩：$C(\pmb{A})=\mathbb{R}^m$ ，即 $\rank\pmb{A}=m$      |
